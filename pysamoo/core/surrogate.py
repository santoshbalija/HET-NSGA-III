...
class Surrogate:
    ...
    def validate(self, trn=None, tst=None, **kwargs):
        for target in self.targets:
            # update the training and test set with cross validation by santosh
            if tst is not None:
                trn = trn.merge(trn, tst)
                tst = None
...
class ProblemFromTargets(MetaProblem):
    ...
    def _evaluate(self, X, out, *args, **kwargs):
        ...
        out["F_sigma"] = np.full((n, self.n_obj), np.nan, dtype=float)
        out["G_sigma"] = np.full((n, self.n_ieq_constr), np.nan, dtype=float)
        out["H_sigma"] = np.full((n, self.n_eq_constr), np.nan, dtype=float)

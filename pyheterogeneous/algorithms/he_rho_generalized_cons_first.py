...
from pyheterogeneous.core.util import pop_to_out, targets_to_pop, all_targets
...
from pyheterogeneous.algorithms.Util_func import select_solutions_with_rho_cons_first_abs
...
class HECF(Algorithm):
    ...
    def _setup(self, problem, **kwargs):
        ...
        self.fillup_time = Tau * self.n_infills
    ...
    def _advance(self, **kwargs):
        ...
        # otherwise perform a step-wise hetregenous evaluation
            ...
            # print("tau:fill_up_time:", self.fillup_time)
            # print("time left", self.termination.max_time - self.scheduler.time())
            # print("Time elapsed:", self.scheduler.time())
            ...
            if self.scheduler.time() <= (self.termination.max_time - self.fillup_time):
                ...
                obj_counts = [infills_selected_obj.count(i) for i in range(n_obj)]

                a, b = np.unique(np.array(infills_selected_idx), return_index=True)
                # print(infills_selected_cons)
                selected_cons_count = [infills_selected_cons[i] for i in b]
                selected_cons_count = [item for sublist in  selected_cons_count if sublist for item in sublist]
                cons_counts = [selected_cons_count.count(i) for i in range(n_constr)]
                for i in range(n_obj):
                    print(f"f{i + 1}-selected-count: {obj_counts[i]}")
                for i in range(n_constr):
                    print(f"g{i + 1}-selected-count: {cons_counts[i]}")
                ...
                for k in range(len(infills_selected)):
                    ...
                    # the targets which are evaluated in the current order iteration
                        targets, _ = self.problem.eval_times[infills_selected_obj[k]]
                    ...
                    for cons in infills_selected_cons[k]:
                        ...
                        # the targets which are evaluated in the current order iteration
                            targets, _ = self.problem.eval_times[cons]
            ...
            # this step is just for presentation of final solutions
            else:
                ...
                print('this step is just for the presentation of final solutions')
    ...
    def _surr_infill(self):
        ...
        # extract the offsprings from the final population (check to be not a duplicate from the current population)
        # pred_off = DefaultDuplicateElimination().do(algorithm.pop, pred_pop)
    ...
    def _prob_independent_evaltime_infill_alpha_time(self):
        ...
        # if self.n_infills is not None and len(pred_pop) > self.n_infills:
        #     pred_off = algorithm.survival.do(problem, pred_pop, n_survive=self.n_infills)
        ...
        infills_selected, infills_selected_idx, infills_selected_obj, infills_selected_cons = select_solutions_with_rho_cons_first_abs(Rho_et,  Prob_r_constr_stack,
                                                                                                 required_solutions,
                                                                                                 ref_dirs, Zg,  e)

# Problems from https://reader.elsevier.com/reader/sd/pii/S1568494620300181?token=0F2E8F437C4C380534C8B9CA54E0CE874AEF70B4B122FD0B349861AE35B8CB881DBCA03265F0D496D876C56B3F213F1D&originRegion=us-east-1&originCreation=20220926181433
...
class ZDT(Problem):
    ...
    def __init__(self, n_var=30, **kwargs):
        super().__init__(n_var=n_var, n_obj=2, vtype=float, **kwargs)
...
class LZDT1(ZDT):
    ...
    def __init__(self, n_var=10):
        self.n_var = n_var
        self.mat = np.load("problems/mat_{}.npy".format(self.n_var))

        x_pf = self._calc_pareto_set()
        xl = np.min(x_pf, axis=0)
        xu = np.max(x_pf, axis=0)
        del x_pf
        super(LZDT1, self).__init__(n_var=self.n_var, xl=xl, xu=xu)
    ...
    def _calc_pareto_set(self, n_pareto_points=500):
        x = np.linspace(0, 1, n_pareto_points).reshape(-1, 1)
        x = np.concatenate([x, np.zeros((n_pareto_points, self.n_var - 1))], axis=1)

        newxs = []
        for i in range(x.shape[0]):
            newxs.append(np.linalg.inv(self.mat) @ x[i])
        x2 = np.row_stack(newxs)
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        newxs = []
        for i in range(x.shape[0]):
            newxs.append(self.mat @ x[i])
        x = np.row_stack(newxs)
        f1 = x[:, 0]
...
class L1(Problem):
    def __init__(self, n_var=10):
        self.n_var = n_var
        super(L1, self).__init__(n_var=self.n_var, n_obj=2,
                                 xl=np.zeros((n_var)), xu=np.ones((n_var)))

    def _evaluate(self, X, out, *args, **kwargs):
        f1 = (1 + self.g(X)) * X[:, 0]
        f2 = (1 + self.g(X)) * (1.0 - (X[:, 0]) ** 0.5)
    ...
    def g(self, X):
        ...
        for i in range(1, self.n_var):
            g += np.abs((X[:, i] - (0.2 + 0.6 * X[:, 0]) ** 2)) ** 0.6
...
class L2(Problem):
    def __init__(self, n_var=10):
        ...
        super(L2, self).__init__(n_var=self.n_var, n_obj=2,
                                 xl=np.zeros((n_var)), xu=np.ones((n_var)))
    ...
    def g(self, X):
        ...
        for i in range(1, self.n_var):
            if i % 2 == 0:
                g += np.abs(X[:, i] - (0.2 + 0.6 * np.sin(np.pi * 0.5 * X[:, 0])))
            else:
                g += np.abs(X[:, i] - (0.2 + 0.6 * np.cos(np.pi * 0.5 * X[:, 0])))

        return g


class Water(Problem):
    # Implemented as described in https://www.tandfonline.com/doi/pdf/10.1080/03052150108940926?needAccess=true

    def __init__(self):
        super(Water, self).__init__(n_var=3, n_obj=5, n_ieq_constr=7,
                                    xl=np.array([0.01, 0.01, 0.01]), xu=np.array([0.45, 0.1, 0.1]))

    def _evaluate(self, X, out, *args, **kwargs):
        # Created by Ritam Guha
        """
...
class Crashworthiness(Problem):
    # Implemented as described in https://link.springer.com/content/pdf/10.1007/s00158-007-0163-x.pdf

    def __init__(self):
        super(Crashworthiness, self).__init__(n_var=5, n_obj=3,
    ...
    def _evaluate(self, X, out, *args, **kwargs):
        ...
        Mass = 1640.2823 + 2.3573285 * X[:, 0] + 2.3220035 * X[:, 1] + 4.5688768 * X[:, 2] + 7.7213633 * X[:,
                                                                                                         3] + 4.4559504 \
               * X[:, 4]

        Ain = 6.5856 + 1.15 * X[:, 0] - 1.0427 * X[:, 1] + 0.9738 * X[:, 2] + 0.8364 * X[:, 3] - 0.3695 * X[:, 0] * X[:,
                                                                                                                    3] \
              + 0.0861 * X[:, 0] * X[:, 4] + 0.3628 * X[:, 1] * X[:, 3] - 0.1106 * (X[:, 0] ** 2) - 0.3437 * (
                      X[:, 2] ** 2) \
              + 0.1764 * (X[:, 3] ** 2)

        Intrusion = -0.0551 + 0.0181 * X[:, 0] + 0.1024 * X[:, 1] + 0.0421 * X[:, 2] - 0.0073 * X[:, 0] * X[:,
                                                                                                          1] + 0.0204 * \
                    X[:, 1] * X[:, 2] - 0.0118 * X[:, 1] * X[:, 3] - 0.0204 * X[:, 2] * X[:, 3] - 0.008 * X[:, 2] * X[:,
                                                                                                                    4] \
...
class Fourbartruss(Problem):
    # An Easy-to-use Real-world Multi-objective Optimization Problem Suite Ryoji Tanabe, Hisao Ishibuchi
    ...
    def __init__(self):
        ...
        L = 200.0
        a =  F_ / sigma
        super(Fourbartruss, self).__init__(n_var=4, n_obj=2,
                                              xl=np.array([a, np.sqrt(2.0)*a, np.sqrt(2.0)*a, a]),
                                              xu=np.array([3*a, 3*a, 3*a, 3*a]))
...
class Rocketinjector(Problem):
    # An Easy-to-use Real-world Multi-objective Optimization Problem Suite Ryoji Tanabe, Hisao Ishibuchi

    def __init__(self):

        super(Rocketinjector, self).__init__(n_var=4, n_obj=3,
                                              xl=np.array([0, 0, 0, 0]),
                                              xu=np.array([1, 1, 1, 1]))
    ...
    def _evaluate(self, X, out, *args, **kwargs):
        ...
        # f1 (TF_max)
        f_0 = 0.692 + (0.477 * xAlpha) - (0.687 * xHA) - (0.080 * xOA) - (0.0650 * xOPTT) - (
                    0.167 * xAlpha * xAlpha) - (
        ...
        # f2 (X_cc)
        f_1 = 0.153 - (0.322 * xAlpha) + (0.396 * xHA) + (0.424 * xOA) + (0.0226 * xOPTT) + (
                    0.175 * xAlpha * xAlpha) + (
        ...
        # f3 (TT_max)
        f_2 = 0.370 - (0.205 * xAlpha) + (0.0307 * xHA) + (0.108 * xOA) + (1.019 * xOPTT) - (
                    0.135 * xAlpha * xAlpha) + (
...
class GAA(Problem):
    # Created by Sukrit Mittal
    def __init__(self):
        super().__init__(n_var=27, n_obj=10, n_constr=18, type_var=np.double)

...
class Evaluator:
    ...
    def eval(self,
        ...
        if count_evals:
            # if the target values are evaluated in batch the number of evaluations is
            # multiplied by the number of objectives
            if kwargs.get('target_values') == 'batch':
                self.n_eval += len(I)*problem.n_obj
            else:
                self.n_eval += len(I)
